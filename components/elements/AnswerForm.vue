...
async function postAnswer() {
  ...
  const answer = await useFetch(
      () => `/api/ask-sami/answer`, { method: 'post', body: { data } }
  );
...
<template>
  <form @submit.prevent="postAnswer">
    ...
    <div class="flex justify-end">
      <button type="submit"
              class="mt-5 px-6 py-3.5 text-white bg-gradient-to-r from-indigo-500 via-indigo-600 to-indigo-700 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-indigo-300 dark:focus:ring-indigo-800 font-medium rounded-lg text-sm text-center mr-2 mb-2">
        Submit
      </button>
      <button @click="showAnswerForm = !showAnswerForm" type="button"
              class="mt-5 px-6 py-3.5 text-white bg-gradient-to-r from-red-500 via-red-600 to-red-700 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-indigo-300 dark:focus:ring-indigo-800 font-medium rounded-lg text-sm text-center mr-2 mb-2">
        Cancel
      </button>
    </div>
  </form>
