...
export async function validateRegisterForm(data: RegistrationRequest) {
    const errors = new Map<string, { check: InputValidation}>()

    for (const [key, value] of Object.entries(data)) {
        let val = await runRegisterCheck(key, value)
        if (val.hasError) {
...
export async function validateLoginForm(data: RegistrationRequest) {
    const errors = new Map<string, { check: InputValidation}>()

    for (const [key, value] of Object.entries(data)) {
        let val = await runLoginCheck(key, value)
        if (val.hasError) {
            errors.set(key, { 'check': val })
        }
    }
...
async function runRegisterCheck(key: string, value: string): Promise<InputValidation> {
...
async function runLoginCheck(key: string, value: string): Promise<InputValidation> {
    const check: InputValidation = {
        value,
        isBlank: false,
        lengthMin8: true,
